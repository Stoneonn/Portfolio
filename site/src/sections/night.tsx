...
export function Built() {
  return (
    <section id="built" data-hour="9" className={`${SHELL} mt-36`}>
      <Stamp time="09:00" title="Built" />
        ...
        {SOCIALS.map((s) => {
          ...
          return (
            <a
              ...
              aria-label={s.label}
              className="c-soft -m-2 p-2 transition-colors hover:text-(--fg)"
            >
              {Icon ? <Icon /> : s.label}
            </a>
          )
...
export function Ending() {
  ...
  return (
    <section
      id="painting"
      data-hour="27.2"
      className="mt-32 flex min-h-[100svh] flex-col items-center justify-center gap-7 px-6 pt-4 pb-9 md:gap-10 md:pt-14 md:pb-2"
    >
      <blockquote className="rv max-w-2xl text-center">
        <p className="serif c-fg text-base leading-snug md:text-xl">“{QUOTE.text}”</p>
        {/* attribution removed by request; its vertical space stays */}
        <div aria-hidden className="mt-4 h-3" />
      </blockquote>
      ...
      <div className="rv w-full">
        ...
        <div className="relative">
          ...
          <img
            ...
            className="relative mx-auto block h-auto max-h-[44svh] w-auto max-w-[86vw]"
          />
        </div>
        <div className="mt-6 text-center">
          <p className="deck c-fg text-lg">{painting.title}</p>
          <p className="serif c-soft mt-0.5 text-lg">{painting.artist}</p>
        </div>
      </div>

      <p className="mono c-soft text-[9px] tracking-[0.22em]">built with {'<3'}</p>
    </section>
  )
}
